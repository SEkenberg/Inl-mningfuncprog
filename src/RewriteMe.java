...
public class RewriteMe {
    ...
    public List<String> getAllAnswerOptionsDistinct(){
        return questions.stream()
                .flatMap(question -> question.getAllAnswers().stream()).distinct().collect(Collectors.toList());
    ...
    public boolean isThisAnAnswerOption(String answerCandidate){
        ...
        return questions.stream().map(Question::getAllAnswers).flatMap(Collection::stream)
                .anyMatch(e -> e.equalsIgnoreCase(answerCandidate));
    ...
    public int getAnswerCandidateFrequncy(String answerCandidate){
        ...
        return (int) questions.stream().map(Question::getAllAnswers)
                .flatMap(Collection::stream).filter(e -> e.equals(answerCandidate)).count();
    }
    ...
    public String getLongestLettercountAnwerInAGivenCategory(Category c)
    {
        return questions.stream()
                ...
                .flatMap(Collection::stream).max(Comparator.comparingInt(String::length)).get();
